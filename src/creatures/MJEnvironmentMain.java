...
public class MJEnvironmentMain extends SimpleApplication implements AnalogListener, ActionListener{
    ...
    private float maxpopulation = 200;
    private float maxsurvivals = 20;
    private int maxparallelchecks = 10;
    ...
    private void Reproduce(boolean detachphysics)
    {
        ...
        //populationselection.get(0).PrintBrain(populationselection.get(0).GetBrain());
        /*----Mark all selection to be killed on next 2 generations----------*/
        populationgarbage.clear();
        
        /*for (int j = 0 ; j < populationselection.size(); j++)
        {
            //populationselection.get(j).PrintBrain(populationselection.get(j).GetBrain());
            if (!populationselection.get(j).IsMarkedToKill()){
                populationselection.get(j).MarkToKill(generation + 2);
            }
        ...
        */
        ...
        // kill creatures that we are not going to use
        for (int j = maxcountsurvivals ; j < populationselection.size(); j++)
        {
            //populationselection.get(j).PrintBrain(populationselection.get(j).GetBrain());
            if (!populationselection.get(j).IsMarkedToKill()){
                populationselection.get(j).MarkToKill(generation);
            }
            populationgarbage.add(populationselection.get(j)); 
        }
        
        /*----Fill the entire population ---- to prevent round issues - the order here is 1 --------*/
        //while(population.size() < (maxpopulation * 1.3f))
        while(population.size() < (maxpopulation))
        {
            Creature creat;
            if (state == 1) //not init state - evolution
            {
                //Add a new Creature
    ...
    public void simpleUpdate(float tpf) {
        ...
        if (populationobserved.size() > 0)
        {
                        ...
                        for (int i = 0 ; i< populationobserved.size(); i++)
                        {
                            ...
                            switch(typeoffitness)//only follow a target
                            {
                                case 2:
                                {
                                    ...
                                    System.out.println("Cool New distanceeeeeeee!!!: " + dist);
                                    populationselection.add(populationobserved.get(i));
            ...
            for (int i = 0; i < populationobserved.size(); i++)
            {
                populationobserved.get(i).GetPhysicState().update(1f, 1);
                
                if ((totalsteps % 200) == 25)
                {
                ...
                if (!populationobserved.get(i).isStartticking())
                {
                    if (!populationobserved.get(i).IsMoving())
                    {
                        ...
                        System.out.print("Start " + i + " Brain! totalsteps:" + totalsteps + "\n");
